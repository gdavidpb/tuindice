...
class QuarterDataRepositoryTest {
    ...
    fun setSubjectGrade_whenDispatchDisabled_updatesLocalOnly() = runBlocking {
        val updatedQuarters = listOf(
            RecordFixtures.localQuarter(
                id = "q1",
                subjects = listOf(RecordFixtures.localSubject(id = "s1", quarterId = "q1", grade = 4))
            )
        )
        localDataSource.computeSetSubjectGradeResult = updatedQuarters
        ...
        assertEquals(1, localDataSource.computeSetSubjectGradeCalls.size)
        assertEquals(updatedQuarters, localDataSource.savedQuartersCalls.last())
    ...
    fun setSubjectGrade_whenDispatchEnabled_updatesRemoteWithUpdatedQuarter() = runBlocking {
        val updatedQuarters = listOf(
            RecordFixtures.localQuarter(
                id = "q1",
                subjects = listOf(RecordFixtures.localSubject(id = "s-target", quarterId = "q1", grade = 5))
            ),
            RecordFixtures.localQuarter(
                id = "q2",
                subjects = listOf(RecordFixtures.localSubject(id = "s-2", quarterId = "q2", grade = 3))
            )
        )
        localDataSource.computeSetSubjectGradeResult = updatedQuarters
    ...
    @Test
    fun setSubjectGrade_whenDispatchEnabledWithoutMatchingSubject_throwsNoSuchElementException() = runBlocking {
        localDataSource.computeSetSubjectGradeResult = listOf(
            RecordFixtures.localQuarter(
                id = "q1",
                subjects = listOf(RecordFixtures.localSubject(id = "different-subject", quarterId = "q1"))
            )
        )
        ...
        assertEquals(1, localDataSource.savedQuartersCalls.size)
        assertTrue(remoteDataSource.addQuarterCalls.isEmpty())
    }
}