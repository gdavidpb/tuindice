...
- Dispatchers live only at the MVI boundary and at blocking leaves; intermediate layers inherit context. Do not add `TuIndiceDispatchers` parameters or `flowOn(...)` calls to use cases, action processors, or repositories. The single `flowOn(dispatchers.default)` in `BaseViewModel` already moves the whole upstream pipeline (processors, use cases, mappers) off the main thread, and injection there is the only substitution point tests need (there is no `Dispatchers.setDefault`). Inject `TuIndiceDispatchers` only into `ViewModel` subclasses (passed through to `BaseViewModel`), `BufferedEventPublisher`, and concrete `DataSource` classes that perform real blocking or CPU-heavy work.
