...
- If you need a fast starting point, use `scripts/scaffold_feature_module.py` before hand-writing boilerplate. It can scaffold a baseline feature module, integrate that feature into the repo wiring, and render templates for `ViewModel`, `Route`, `ScreenMachine` (machine + internal events + transitions + error mapper + machine contract test), `UseCase`, `Repository`, `Navigation`, `Screen`, and smoke tests.
...
- `ViewModel` classes extend `StateMachineViewModel` and stay a pure screen API: one-line `sendAction` helpers, no `MutableStateFlow`s, no use cases, no guards reading `state.value`. The machine enters as `override val screenMachine: <Screen>Machine` via Koin (`factoryOf`).
- The screen anatomy is `presentation/machine/<Screen>Machine.kt` (Koin-injectable class owning use cases, `initialState()`, `define()` and async commands), `presentation/machine/<Screen>InternalEvent.kt` (async results re-enter the table as input symbols, split per outcome so every row keeps a fixed declared target), and `presentation/transition/<Screen><FromState>Transitions.kt` (builder extension functions; one file per from-state block plus `AnyState` when needed).
- Minimal payloads: an `Action` carries only new information from the environment (typed text, navigation arguments). Anything the state already knows is read from `state` inside the row — `f: S x E -> S` receives S. State-dependent validity lives in the table (a state-specific row; other states reject with `app_invalid_transition` telemetry), never as a `?: return` guard in a ViewModel helper. Guards over extended state (payload fields) live in the machine command, documented as EFSM guards.
- Form screens follow the draft pattern (`CreateSyntheticTerm`, `SubjectSearch`): input registers (`MutableStateFlow`s feeding observation pipelines) are reified as `presentation/machine/<Screen>Draft.kt`, injected only into the machine, written only from machine level (rows and commands). Every user gesture is an `Action` with a table row.
- Effects are declared per row via `emits` (may-emit); the Λ-coverage validator runs with an empty `except` set — dead effect symbols get deleted, not excepted.
- Error-type to text mapping functions live in `presentation/mapper` (`<Screen>ErrorMessages.kt`); machines and transitions call them. Fixed single-resource messages built inline in a row or command are data, not mapping functions, and may stay inline.
- Every machine ships a `<Screen>StateMachineContractTest`: alphabet coverage, state reachability, Λ coverage, and a Mermaid export assertion. Build the fixture manually when the machine has few use cases or the test drives behavior with controllable fakes; boot the real Koin module with stubbed repositories (`withKoinSmokeTest`) when hand-building the use-case graph gets heavy (see `RecordStateMachineContractTest`).
- Dispatchers live only at the MVI boundary and at blocking leaves; intermediate layers inherit context. Do not add `TuIndiceDispatchers` parameters or `flowOn(...)` calls to use cases, machines, or repositories. The single `flowOn(dispatchers.default)` in `StateMachineViewModel` already moves the whole upstream pipeline (machine jobs, use cases, mappers) off the main thread, and injection there is the only substitution point tests need (there is no `Dispatchers.setDefault`). Inject `TuIndiceDispatchers` only into `ViewModel` subclasses (passed through to `StateMachineViewModel`), `BufferedEventPublisher`, and concrete `DataSource` classes that perform real blocking or CPU-heavy work.
...
- For repository-backed feature state, prefer the `summary` and `record` split:
  ...
  - the machine's `startObservation` command maps the observed flow to internal events; the table rows own the reduction
  - the machine's `refresh` command owns startup/retry refresh, with error messages resolved through the mapper before the internal event re-enters the table
