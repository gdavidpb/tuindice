package com.gdavidpb.tuindice.pensum.presentation.action

import com.gdavidpb.tuindice.base.presentation.model.UiText
import com.gdavidpb.tuindice.pensum.domain.model.PensumObservation
import com.gdavidpb.tuindice.pensum.domain.repository.PensumRepository
import com.gdavidpb.tuindice.pensum.domain.usecase.SelectPensumUseCase
import com.gdavidpb.tuindice.pensum.domain.usecase.exceptionhandler.UpdatePensumExceptionHandler
import com.gdavidpb.tuindice.pensum.presentation.contract.Pensum
import com.gdavidpb.tuindice.pensum.presentation.model.PensumCanvasItem
import com.gdavidpb.tuindice.pensum.presentation.model.PensumScreenModel
import com.gdavidpb.tuindice.pensum.presentation.model.PensumScreenSelection
import com.gdavidpb.tuindice.testkit.base.repository.FakeNetworkRepository
import com.gdavidpb.tuindice.testkit.base.repository.RecordingReportingRepository
import com.gdavidpb.tuindice.testkit.ktor.clientRequestException
import com.gdavidpb.tuindice.testkit.ktor.serverResponseException
import com.gdavidpb.tuindice.testkit.mvi.reduceMutations
import io.ktor.http.HttpStatusCode
import kotlin.test.Test
import kotlin.test.assertEquals
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.emptyFlow
import kotlinx.coroutines.flow.toList
import kotlinx.coroutines.test.runTest
import tuindice.pensum.generated.resources.Res
import tuindice.pensum.generated.resources.pensum_failed_service_unavailable
import tuindice.pensum.generated.resources.pensum_local_data_warning_service

class SelectPensumActionProcessorTest {
	@Test
	fun when_selectPensumStartsWhileContentIsVisible_then_refreshingFlagIsEnabledAndWarningIsCleared() = runTest {
		val processor = createProcessor(pensumRepository = RecordingSelectPensumRepository())
		val contentState = selectPensumContentState(
			localDataMessage = UiText.Resource(Res.string.pensum_local_data_warning_service)
		)
		val effects = mutableListOf<Pensum.Effect>()

		val mutations = processor.process(
			action = Pensum.Action.SelectPensum(year = 2017),
			sideEffect = effects::add
		).toList()
		val loadingState = mutations.first().invoke(contentState)

		assertEquals(
			contentState.copy(isRefreshing = true, localDataMessage = null),
			loadingState
		)
		assertEquals(emptyList(), effects)
	}

	@Test
	fun when_selectPensumSucceedsWhileContentIsVisible_then_refreshingFlagIsDisabled() = runTest {
		val processor = createProcessor(pensumRepository = RecordingSelectPensumRepository())
		val contentState = selectPensumContentState()
		val effects = mutableListOf<Pensum.Effect>()

		val finalState = processor.process(
			action = Pensum.Action.SelectPensum(year = 2017),
			sideEffect = effects::add
		).toList().reduceMutations(contentState)

		assertEquals(
			contentState.copy(isRefreshing = false),
			finalState
		)
		assertEquals(emptyList(), effects)
	}

	@Test
	fun when_selectPensumSucceedsWhileLoading_then_stateStaysLoading() = runTest {
		val processor = createProcessor(pensumRepository = RecordingSelectPensumRepository())
		val effects = mutableListOf<Pensum.Effect>()

		val finalState = processor.process(
			action = Pensum.Action.SelectPensum(year = 2017),
			sideEffect = effects::add
		).toList().reduceMutations(Pensum.State.Loading)

		assertEquals(Pensum.State.Loading, finalState)
		assertEquals(emptyList(), effects)
	}

	@Test
	fun when_selectPensumReturnsNotFound_then_stateBecomesEmptyWithoutSnackbar() = runTest {
		val processor = createProcessor(
			pensumRepository = ThrowingSelectPensumRepository(
				throwable = clientRequestException(
					statusCode = HttpStatusCode.NotFound,
					path = "/pensums/v4"
				)
			)
		)
		val effects = mutableListOf<Pensum.Effect>()

		val finalState = processor.process(
			action = Pensum.Action.SelectPensum(year = 2017),
			sideEffect = effects::add
		).toList().reduceMutations(Pensum.State.Loading)

		assertEquals(Pensum.State.Empty, finalState)
		assertEquals(emptyList(), effects)
	}

	@Test
	fun when_selectPensumReturnsServiceUnavailable_then_stateBecomesFailed() = runTest {
		val processor = createProcessor(
			pensumRepository = ThrowingSelectPensumRepository(
				throwable = serverResponseException(
					statusCode = HttpStatusCode.ServiceUnavailable,
					path = "/pensums/v4"
				)
			)
		)
		val effects = mutableListOf<Pensum.Effect>()

		val finalState = processor.process(
			action = Pensum.Action.SelectPensum(year = 2017),
			sideEffect = effects::add
		).toList().reduceMutations(Pensum.State.Loading)

		assertEquals(
			Pensum.State.Failed(
				message = UiText.Resource(Res.string.pensum_failed_service_unavailable)
			),
			finalState
		)
		assertEquals(emptyList(), effects)
	}

	@Test
	fun when_selectPensumFailsWhileContentIsVisible_then_contentRemainsAndLocalDataWarningIsShown() = runTest {
		val processor = createProcessor(
			pensumRepository = ThrowingSelectPensumRepository(
				throwable = serverResponseException(
					statusCode = HttpStatusCode.ServiceUnavailable,
					path = "/pensums/v4"
				)
			)
		)
		val contentState = selectPensumContentState()
		val effects = mutableListOf<Pensum.Effect>()

		val finalState = processor.process(
			action = Pensum.Action.SelectPensum(year = 2017),
			sideEffect = effects::add
		).toList().reduceMutations(contentState)

		assertEquals(
			contentState.copy(
				localDataMessage = UiText.Resource(Res.string.pensum_local_data_warning_service)
			),
			finalState
		)
		assertEquals(emptyList(), effects)
	}

	@Test
	fun when_selectPensumExecutes_then_yearIsForwardedToRepository() = runTest {
		val pensumRepository = RecordingSelectPensumRepository()
		val processor = createProcessor(pensumRepository = pensumRepository)

		processor.process(
			action = Pensum.Action.SelectPensum(year = 2017),
			sideEffect = {}
		).toList()

		assertEquals(listOf(2017), pensumRepository.selectedYears)
	}
}

private fun createProcessor(
	pensumRepository: PensumRepository,
	isNetworkAvailable: Boolean = true
): SelectPensumActionProcessor {
	return SelectPensumActionProcessor(
		selectPensumUseCase = SelectPensumUseCase(
			pensumRepository = pensumRepository,
			reportingRepository = RecordingReportingRepository(),
			exceptionHandler = UpdatePensumExceptionHandler(
				networkRepository = FakeNetworkRepository(isAvailable = isNetworkAvailable)
			)
		)
	)
}

private class RecordingSelectPensumRepository : PensumRepository {
	val selectedYears = mutableListOf<Int>()

	override fun observePensumFlow(): Flow<PensumObservation> = emptyFlow()

	override suspend fun refreshPensum() = Unit

	override suspend fun selectPensum(year: Int) {
		selectedYears += year
	}

	override suspend fun selectModality(modalityId: String) = Unit

	override suspend fun selectSelection(year: Int, modalityId: String) = Unit
}

private class ThrowingSelectPensumRepository(
	private val throwable: Throwable
) : PensumRepository {
	override fun observePensumFlow(): Flow<PensumObservation> = emptyFlow()

	override suspend fun refreshPensum() = Unit

	override suspend fun selectPensum(year: Int) {
		throw throwable
	}

	override suspend fun selectModality(modalityId: String) = Unit

	override suspend fun selectSelection(year: Int, modalityId: String) = Unit
}

private fun selectPensumContentState(
	localDataMessage: UiText? = null
): Pensum.State.Content {
	return Pensum.State.Content(
		model = PensumScreenModel(
			careerName = "Ingenieria de Computacion",
			selection = PensumScreenSelection(
				year = 2019,
				modalityId = "degree_project"
			),
			pensumOptions = emptyList(),
			modalityOptions = emptyList(),
			progressPercent = 0,
			approvedCredits = 0,
			totalCredits = 0,
			isCurrentFocusVisible = false,
			canvas = PensumCanvasItem(width = 0.0, height = 0.0),
			terms = emptyList(),
			nodes = emptyList(),
			edges = emptyList()
		),
		localDataMessage = localDataMessage
	)
}
